...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Which desktop submenu is showing. Driven by state rather than CSS
    // :hover/:focus-within so the submenu is genuinely absent from the DOM when
    // closed — a visibility:hidden submenu can't receive focus, which left these
    ...
    useEffect(() => {
        ...
        const sections = ['about', 'faq', 'apply', 'offer', 'platform', 'case-study', 'process', 'connect', 'benefits', 'requirements'];
    ...
    const primaryNav = [
        ...
        },
        {
            ...
            path: '/partners',
            links: [
                { name: 'What We Offer', href: '/partners#offer', id: 'offer' },
                { name: 'Case Study', href: '/partners#case-study', id: 'case-study' },
                { name: 'Partnership Process', href: '/partners#process', id: 'process' },
                { name: 'Get in Touch', href: '/partners#connect', id: 'connect' },
            ],
        },
        {
            ...
            path: '/universities',
            links: [
                { name: 'Benefits', href: '/universities#benefits', id: 'benefits' },
                { name: 'Requirements', href: '/universities#requirements', id: 'requirements' },
                { name: 'Partnership Process', href: '/universities#process', id: 'process' },
                { name: 'Get in Touch', href: '/universities#connect', id: 'connect' },
            ],
        },
    ];
        ...
        <div className="min-h-screen flex flex-col bg-black text-white selection:bg-white selection:text-black">
            ...
            <nav aria-label="Main navigation" className={`fixed w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-black/80 backdrop-blur-md border-b border-white/30' : 'bg-transparent'}`}>
                <div ref={navBarRef} className="container mx-auto px-4 md:px-6 py-4 flex justify-between items-center">
                    ...
                    <ul className="hidden md:flex space-x-8 text-sm font-medium list-none m-0 p-0">
                        {primaryNav.map((item) => (
                            <li
                                key={item.path}
                                ...
                                onBlur={(e) => {
                                    // Close once focus leaves the item entirely.
                                    if (!e.currentTarget.contains(e.relatedTarget as Node)) setOpenMenu(null);
                                }}
                                onKeyDown={(e) => {
                                    if (e.key === 'Escape' && openMenu === item.path) {
                                        setOpenMenu(null);
                                        (e.currentTarget.querySelector('button') as HTMLElement | null)?.focus();
                                    }
                                }}
                            >
                                {/* The page link is just a link — it navigates, it doesn't pop up. */}
                                <Link
                                    to={item.path}
                                ...
                                {/* Separate control owns the disclosure, so aria-expanded
                                    describes something that actually expands. */}
                                <button
                                    type="button"
                                    aria-expanded={openMenu === item.path}
                                    aria-label={`${item.name} sections`}
                                    onClick={() => setOpenMenu(openMenu === item.path ? null : item.path)}
                                    className="p-1 -m-1 text-zinc-300 hover:text-white transition-colors rounded focus:outline-none focus-visible:ring-2 focus-visible:ring-white"
                                >
                                    <svg
                                        className={`w-3 h-3 transition-transform duration-200 ${openMenu === item.path ? 'rotate-180' : ''}`}
                                        viewBox="0 0 24 24"
                                        fill="none"
                                        stroke="currentColor"
                                        strokeWidth={3}
                                ...
                                {openMenu === item.path && (
                                    <div className="absolute left-1/2 -translate-x-1/2 top-full pt-3 z-50">
                                        <ul className="min-w-44 bg-black/90 backdrop-blur-md border border-white/30 rounded-xl shadow-2xl py-2 list-none m-0">
                                            {item.links.map((link) => (
                                                <li key={link.id}>
                                                    <Link
                                                        to={link.href}
                    ...
                    <div className="flex items-center space-x-3 md:space-x-4">
                        <a
                            ...
                            aria-label="Slack channel (opens in a new tab)"
                        >
            ...
            <footer aria-label="Site footer" className="bg-black border-t border-white/30 py-12">
